---
name: re1999-snap
description: re1999-hvideos 帧截图管线：按项目/单元的 frames.json 从输入素材按时间戳提取截图帧（pnpm snap 规格编写、dry-run 纯色帧纠偏预警、按项目/单元提取、--strict、产物核验）。剪辑/导出走 re1999-video-clipping。
---
...
- 每单元规格 `media/work/<项目>/screenshots/<单元>/frames.json`：`{ "screenshots": [ { "id", "source", "at", "format", "dir?" } ] }`
- `at` = 原始素材的**绝对时间戳**（秒数 / `MM:SS` / `HH:MM:SS[.mmm]`），不能 ≥ 源时长（CLI 校验）
- `format`：`jpg`（`-q:v 2`）/ `png`（无损）/ `webp`（`-quality 90`），每条一个
- `dir` 缺省 = work→output 镜像目录，产物默认 = `media/output/<项目>/screenshots/<单元>/{id}.{format}`
- 扫描根默认 `media/work`（screenshots 类）、探针临时目录默认 `media/temp`，可用 `RE1999_WORK_DIR`/`RE1999_TEMP_DIR` 覆盖（ADR-0007/0009 环境配置面，详见 `../../re1999-common/PROJECT.md`）
- `at` 保持**意图时刻**不回写：纠偏后产物文件名不变，实际取帧时刻记在日志
- 项目级规则/素材事实见 `../../re1999-common/PROJECT.md`；截图来源永远是 `media/input` 原片，不是剪辑产物

## 执行流程

1. **预检**：读 `../../../CONTEXT.md`（规则）；`../../../docs/adr/0004-截图帧导出.md` 与 `../../../docs/adr/0005-防纯色帧自动纠偏.md`（取帧/纠偏决策）；`ls` 确认 `frames.json` 与素材路径。
2. **编写/更新 `media/work/<项目>/screenshots/<单元>/frames.json`**。完成标准：每条 `id` 唯一、`at` 在源时长内、`format` 合法。
3. **dry-run 预览（必做）**：`pnpm snap run --project <项目> --unit <单元> --dry-run`。逐条判定 `at` 帧内容并预警 `将自动纠偏至 ~HH:MM:SS.mmm`（25 条约 ~84s，属正常探测成本）。完成标准：无 `WARN/ERROR`；纠偏预警数量与意图一致。
4. **执行提取**：`pnpm snap run [--project <项目>] [--unit <单元>]`。
...
5. **核验产物**：`ls media/output/<项目>/screenshots/<单元>/` 产物数量 = 规格条目数；抽查画面内容与预期场景一致。

## 重跑截图（改时间戳/换格式后的标准做法）

1. 产物可从原片 + 规格随时复现，删除零风险：`rm -f media/output/<项目>/screenshots/<单元>/*.{jpg,png,webp}`
2. 与用户核对/更新 `frames.json`（`at` 保持意图时刻）
3. `pnpm snap run --project 1999 --unit epN --dry-run` 预览
4. `pnpm snap run --project 1999 --unit epN` 执行
5. 核验数量与纠偏日志
...
| `pnpm snap run` | 提取全部项目/单元的截图（自动纠偏） |
| `pnpm snap run --project 1999 --unit ep1` | 只跑某项目某单元（旗标均可省略 = 全量） |
| `pnpm snap run --strict` | 关闭自动纠偏：判坏即单条报错、退出码非零 |
| `pnpm snap list` | 按 项目→单元 两级别出已发现的截图规格 |
...
- **ADR-0009 三段式媒体目录 + 项目级泛化**：规格归 `media/work/<项目>/screenshots/<单元>`，产物默认落 `media/output` 镜像，CLI 引入 `--project`/`--unit`；`RE1999_WORK_DIR`/`RE1999_OUTPUT_DIR` 取代旧旋钮。
- **ADR-0007 环境驱动配置面**：规格扫描根/临时目录可配，默认即 `media/` 三段布局；纠偏窗口等参数不是配置。
